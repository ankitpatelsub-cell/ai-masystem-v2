...
// Lightweight script detection (avoids a Claude call for obvious cases).
...
function understand(t){ const L=(t||'').toLowerCase();
  const m=(L.match(/name is ([a-z]+)|i am ([a-z]+)|मेरा नाम ([a-z]+)|नाम ([a-z]+)|名前は ([a-z]+)|わたしは ([a-z]+)/)||[]);
  const raw=m[1]||m[2]||m[3]||m[4]||m[5]||m[6];
  const name=raw?raw.charAt(0).toUpperCase()+raw.slice(1):'';
  const complaint=(L.match(/fever|pain|दर्द|बुखार|熱|head|सिर|chest|सीने|頭|胸/)||[])[0]||'';
  return { name, complaint }; }

async function intake(text, locale){
  locale = locale || detectLocale(text);
  const L=I18N[locale]||I18N.en;
  // Translate non-English input to EN for the understand() parser.
  let enText = text;
...
router.post('/intake', requirePerm('hospital:chat'), async (req,res)=>{ const {text,locale}=req.body||{}; const o=await intake(text,locale); logActivity('hospital','🏥','Patient intake',(text||'').slice(0,40)); res.json(o); });
