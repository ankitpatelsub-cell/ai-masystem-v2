...
function intake(text, locale){ const L=I18N[locale]||I18N.en; const u=understand(text);
  ...
  return {id, steps}; }
router.post('/intake', requirePerm('hotel:chat'), (req,res)=>{ const {text,locale}=req.body||{}; const o=intake(text,locale); logActivity('hotel','🏨','Hotel booking',(text||'').slice(0,40)); res.json(o); });
