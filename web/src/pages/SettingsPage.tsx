...
export default function SettingsPage() {
  ...
  async function save(p: string) { await api('POST', '/api/set-provider', { provider: p, openrouter_model: orm }); setProv(p); }
  return (
    <>
      ...
      <div className="card"><h3>🧠 AI Model</h3>
        <div className="row">
          ...
          <button className="chip" style={prov === 'auto' ? { background: 'var(--brand)', color: '#fff' } : {}} onClick={() => save('auto')}>Auto (failover)</button>
          <button className="chip" style={prov === 'openrouter' ? { background: 'var(--brand)', color: '#fff' } : {}} onClick={() => save('openrouter')}>OpenRouter</button>
        </div>
